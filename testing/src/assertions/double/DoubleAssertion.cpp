...
NotDoubleAssertion DoubleAssertion::not() const
{
	return NotDoubleAssertion(value);
}
...
void DoubleAssertion::toBeAlmostEqualTo(double other) const
{
	...
	if (isAlmostEqual) {
		std::string message = "Expected " + std::to_string(value) + " to be almost equal to " + std::to_string(other);
...
{
	if (value >= other) {
		std::string message = "Expected " + std::to_string(value) + " to be less than " + std::to_string(other);
		throw AssertionException(message);
	}
}
...
{
	if (value <= other) {
		std::string message = "Expected " + std::to_string(value) + " to be greater than " + std::to_string(other);
		throw AssertionException(message);
	}
}
